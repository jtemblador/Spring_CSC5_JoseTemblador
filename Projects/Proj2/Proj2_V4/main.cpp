...
int main(int argc, char** argv) {
    ...
    switch(sort)
    {
        case 1: selcSrt(crdnms, cardnum1);break;
        case 2: bublSrt(crdnms, cardnum1);break;
    }
    
    cout<<endl;
    printHand(crdnms, allcards, cardnum1);
...
void printHand(int crdnms[], string allcards[], int cardnum1) {
    for(int i=1;i<cardnum1;i++){
        cout<<"Card number "<<i<<" is now "<<allcards[crdnms[i]]<<"."<<endl;
    }
    cout<<endl;
}
...
void bublSrt(int a[],int n){
    ...
    do{
        swap=false;
        for(int i=1;i<n;i++){  //Loop to swap with first in List
